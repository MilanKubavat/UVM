...
my ($clean, $test_name, $batch_mode, $compile_mode, $gui_mode, $regress, $help);
GetOptions(
    "clean"     => \$clean,
    "test=s"    => \$test_name,
    "batch"     => \$batch_mode,
...
if(defined $help){
   print("Run Example: perl run.pl -test=testname");
   exit;
}
...
sub compile(){
    system "vlog -work $WORK -f uart_inc.f +define+UVM_NO_DPI";
}
...
sub report(){
   system "vcover merge cov_final *_cov";
